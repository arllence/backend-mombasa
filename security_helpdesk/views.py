...
from security_helpdesk import models
from security_helpdesk import serializers
from security_helpdesk.utils import shared_fxns
    ...
    def issues(self, request):
        ...
        if request.method == "POST":
            ...
            if serializer.is_valid():
                ...
                # track user
                try:
                    track_user.get_client_info(request,'security_helpdesk', uid)
                except Exception as e:
                    logger.error(e)
                ...
                try:
                    if not forwarded:
                        ...
                        facility = Facility.objects.filter(name__icontains=facility).first()
                except Exception as e:
                    logger.error(e)
                    facility = None
                ...
                with transaction.atomic():
                    ...
                    subject = f"[SECURITY HELPDESK] {subject}"
                    message = f"""
                    ...
                    link = "http://172.20.0.42:8015/" + uri
        ...
        if request.method == "POST":
            ...
            if serializer.is_valid():
                ...
                with transaction.atomic():
                    ...
                    subject = f"[SECURITY HELPDESK] Ticket {issueInstance.uid}  Status"
                    if action == 'DONE':
        ...
        if request.method == "POST":
            ...
            if serializer.is_valid():
                ...
                try:
                    ...
                    pass
                subject = f"[SECURITY HELPDESK] Note Issued for {issueInstance.uid}"
...
class HelpDeskViewSet(viewsets.ViewSet):
    ...
    def issue(self, request):
        ...
        if request.method == "POST":
            ...
            # track user
            try:
                track_user.get_client_info(request,'security_helpdesk')
            except:
                pass
        ...
        if request.method == "PUT":
            ...
            if serializer.is_valid():
                ...
                with transaction.atomic():
                    ...
                    # Notify Platform Admins
                    emails = []
                    subject = f"[SECURITY HELPDESK] Ticket {issueInstance.uid} Closed "
                    ...
                    if not is_completed:
                        try:
                            ...
                            subject = f"[SECURITY HELPDESK] Issue {issueInstance.uid}  Completed "
            ...
            if request_id:
                ...
                try:
                    ...
                    if "SECURITY_ADMIN" in roles or "SUPERUSER" in roles:
                        if query == 'unassigned':
                            ...
                            resp = models.Issue.objects.filter(
                                ...
                                Q(security_assignee_issue_instance__assignee=request.user),
    ...
    def assign(self, request):
        ...
        allowed = ["SECURITY_ADMIN", "SUPERUSER"]
        ...
        if request.method == "POST":
            ...
            if serializer.is_valid():
                ...
                with transaction.atomic():
                    ...
                    subject = f"[SECURITY HELPDESK] Issue {issueInstance.uid}  Assigned To You"
                    ...
                    emails = [issueInstance.email or issueInstance.created_by.email]
                    subject = f"[SECURITY HELPDESK] Issue {issueInstance.uid}  Assigned "
        ...
        if request.method == "POST":
            ...
            if serializer.is_valid():
                ...
                with transaction.atomic():
                    ...
                    subject = f"[SECURITY HELPDESK] Issue {issueInstance.uid}  Out Of Scope  "
        ...
        if request.method == "POST":
            ...
            if serializer.is_valid():
                ...
                with transaction.atomic():
                    ...
                    # Notify Admins
                    # emails = list(get_user_model().objects.filter(Q(groups__name__in=['SECURITY_ADMIN'])).values_list('email', flat=True))
                    emails = []
                    assignees = models.Assignees.objects.filter(Q(issue=issueInstance))
                    for assignee in assignees:
                        emails.append(assignee.assignee.email)
                    ...
                    subject = f"[SECURITY HELPDESK] Issue {issueInstance.uid}  Reopened  "
        ...
        if request.method == "POST":
            ...
            if serializer.is_valid():
                ...
                try:
                    ...
                    pass
                subject = f"[SECURITY HELPDESK] Note Issued for Ticket {issueInstance.uid}"
    ...
    def platform_admins(self, request):
        ...
        if request.method == "POST":
            ...
            if serializer.is_valid():
                ...
                # if is_slt or is_hod:
                assign_role = user_util.award_role('SECURITY_ADMIN', str(admin.id))
                if not assign_role:
                    return Response({"details": "Unable to assign role SECURITY_ADMIN"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            if request_id:
                try:
                    ...
                    user_util.revoke_role('SECURITY_ADMIN', str(user.admin.id))
                    user.delete()
                    return Response('200', status=status.HTTP_200_OK)
                except (ValidationError, ObjectDoesNotExist):
                    return Response({"details": "Unknown request"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def documents(self, request):
        ...
        if request.method == "POST":
            ...
            issue_id = payload['issue_id']
            file_type = payload['upload_type']

            try:
                targetInstance = models.Issue.objects.get(id=issue_id)
            except Exception as e:
                return Response({"details": "Unknown request"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            user_util.log_account_activity(
                authenticated_user, authenticated_user, "File uploaded", f"Issue Id: {targetInstance.id}")
            ...
            if request_id:
                ...
                try:
                    resp = models.Document.objects.filter(Q(issue=issue_id))
                ...
                try:
                    resp = models.Document.objects.filter(
                        ...
                        Q(issue__uid__icontains=query) |
                        Q(issue__department__name__icontains=query)
                    )
                ...
                try:
                    ...
                    if any(role in ['SUPERUSER','SECURITY_ADMIN'] for role in roles):
    ...
    def issues(self, request):
        ...
        if r_status:
            if r_status == "QUOTE REQUESTED":
                q_filters &= Q(security_quote_issue_instance__isnull=False)
            else:
...
class AnalyticsViewSet(viewsets.ViewSet):
    ...
    def general(self, request):
        ...
        if any(role in ['FMS_ADMIN', 'SUPERUSER'] for role in roles):
            ...
            requests = models.Issue.objects.filter(Q(department=request.user.srrs_department) | Q(created_by=request.user) | Q(assigned_to=request.user) | Q(security_assignee_issue_instance__assignee=request.user), is_deleted=False).count()
            assigned = models.Issue.objects.filter(Q(department=request.user.srrs_department) |  Q(created_by=request.user) | Q(assigned_to=request.user) | Q(security_assignee_issue_instance__assignee=request.user), status="ASSIGNED", is_deleted=False).count()
            closed = models.Issue.objects.filter(Q(department=request.user.srrs_department) |  Q(created_by=request.user) | Q(assigned_to=request.user) | Q(security_assignee_issue_instance__assignee=request.user), status="CLOSED", is_deleted=False).count()
            pending = models.Issue.objects.filter(Q(department=request.user.srrs_department) |  Q(created_by=request.user) | Q(assigned_to=request.user) | Q(security_assignee_issue_instance__assignee=request.user), status__in=active_status, is_deleted=False).count()
