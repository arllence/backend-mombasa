    ...
    def EquipmentType(self, request):
        ...
        if request.method == "POST":
            ...
            if serializer.is_valid():
                ...
                exists = models.EquipmentType.objects.filter(Q(name=name)).exists()
                if exists:
                    return Response({"details": "Entry already exists"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if request.method == "POST":
            ...
            if request_id:
                ...
                try:
                    ...
                    if any(role in ['SUPERUSER','ICT_ADMIN'] for role in roles):
